...
import { MEDICAL_SYMBOLS, renderMedicalSymbol, getSymbolsByRegulation, CATEGORY_LABELS, type MedicalSymbol } from "@/components/MedicalSymbols";
...
const PRESET_TEMPLATES: LabelTemplate[] = [
  ...
  },
  {
    ...
    elements: [
      ...
      { id: uid(), type: "symbol", x: 5, y: 15, width: 5, height: 5, content: "", symbolId: "ref" },
      ...
      { id: uid(), type: "symbol", x: 5, y: 22, width: 5, height: 5, content: "", symbolId: "lot" },
      { id: uid(), type: "text", x: 12, y: 22, width: 30, height: 5, content: "2024092401", fieldBinding: "batchNo", fontSize: 9, textAlign: "left", color: "#000" },
      { id: uid(), type: "symbol", x: 55, y: 22, width: 5, height: 5, content: "", symbolId: "expiryDate" },
      ...
      { id: uid(), type: "barcode", x: 5, y: 52, width: 60, height: 12, content: "(01)06975573321040", fieldBinding: "gtin", barcodeFormat: "GS1-128", showText: true },
      { id: uid(), type: "qrcode", x: 72, y: 52, width: 14, height: 14, content: "(01)06975573321040(17)260923(10)2024092401", barcodeFormat: "GS1-DATAMATRIX" },
      { id: uid(), type: "text", x: 5, y: 67, width: 90, height: 4, content: "FDA Listing: K123456", fontSize: 7, textAlign: "left", color: "#555" },
      { id: uid(), type: "barcode", x: 5, y: 72, width: 90, height: 7, content: "(01)06975573321101(17)250112(10)20230113011", barcodeFormat: "GS1-128", showText: false },
    ],
  },
  {
    ...
    elements: [
      ...
      { id: uid(), type: "text", x: 5, y: 22, width: 90, height: 5, content: "注册证号：国械注准20231234567", fieldBinding: "registrationNo", fontSize: 8, textAlign: "left", color: "#000" },
      ...
      { id: uid(), type: "barcode", x: 5, y: 56, width: 60, height: 12, content: "(01)06975573321040", fieldBinding: "gtin", barcodeFormat: "GS1-128", showText: true },
      { id: uid(), type: "qrcode", x: 72, y: 56, width: 14, height: 14, content: "(01)06975573321040(17)260923(10)2024092401", barcodeFormat: "GS1-DATAMATRIX" },
...
export default function Home() {
  ...
  function renderElement(el: LabelElement) {
    ...
    if (el.type === "barcode") return (
      <div key={el.id} onMouseDown={e => handleMouseDown(e, el.id)} style={{ ...baseStyle, outline: selBorder, display: "flex", alignItems: "center", justifyContent: "center", overflow: "hidden" }}>
    ...
    if (el.type === "qrcode") return (
      <div key={el.id} onMouseDown={e => handleMouseDown(e, el.id)} style={{ ...baseStyle, outline: selBorder }}>
  ...
  return (
    <div className="h-screen flex flex-col bg-[#f0f2f5] overflow-hidden">
      {/* ===== 顶部工具栏 ===== */}
      <header className="h-11 border-b bg-white flex items-center gap-1.5 px-3 shrink-0 shadow-sm print:hidden">
        <div className="flex items-center gap-2 mr-2">
          <div className="w-7 h-7 rounded-md bg-[#0F2B46] flex items-center justify-center">
            <FileText className="w-3.5 h-3.5 text-white" />
          </div>
          <div className="leading-none">
            <h1 className="text-[12px] font-bold tracking-tight text-gray-800">UDI 标签设计器</h1>
            <p className="text-[9px] text-gray-400 flex items-center gap-0.5"><Database className="w-2.5 h-2.5" />ERP集成</p>
          </div>
        </div>

        <Separator orientation="vertical" className="h-6" />

        <Input value={template.name} onChange={e => setTemplate(t => ({ ...t, name: e.target.value }))} className="w-40 h-7 text-xs" />
        {template.regulation && <Badge variant="outline" className="text-[10px] h-5">{template.regulation}</Badge>}

        <Separator orientation="vertical" className="h-6" />

        <span className="text-[10px] text-gray-400 font-medium">插入:</span>
        {[
        ...
        <div className="flex items-center gap-0.5">
          ...
          <span className="text-[10px] text-gray-400 w-8 text-center tabular-nums">{Math.round(canvasScale * 100)}%</span>
        ...
        <div className="ml-auto flex items-center gap-1.5">
            ...
            <DropdownMenuContent align="end" className="w-56">
              ...
              {savedTemplates.length > 0 && (
                <>
                  ...
                  <div className="px-2 py-1 text-[10px] font-semibold text-gray-400 uppercase">已保存</div>
          ...
          <Button size="sm" className="gap-1 h-7 text-xs bg-[#0F2B46] hover:bg-[#0F2B46]/90" onClick={saveTemplate}><Save className="w-3.5 h-3.5" />保存</Button>
        </div>
      </header>
      ...
      <div className="flex flex-1 overflow-hidden print:block">
        {/* 左侧面板 */}
        <div className="w-52 border-r bg-white flex flex-col shrink-0 overflow-hidden print:hidden">
          <Tabs value={leftTab} onValueChange={setLeftTab} className="flex flex-col h-full">
            <TabsList className="h-8 bg-gray-50 rounded-none border-b shrink-0 w-full justify-start px-1">
            ...
            <TabsContent value="tools" className="flex-1 overflow-y-auto m-0 p-2 space-y-2">
              <p className="text-[10px] font-semibold text-gray-400 uppercase tracking-wide">基础元素</p>
              ...
              <p className="text-[10px] font-semibold text-gray-400 uppercase tracking-wide">快捷操作</p>
            ...
            <TabsContent value="layers" className="flex-1 overflow-y-auto m-0">
              <div className="py-1">
                {template.elements.length === 0 ? (
                  <p className="text-xs text-gray-400 text-center py-8">暂无元素<br />从工具栏添加</p>
                ) : [...template.elements].reverse().map(el => (
                  <div key={el.id} onClick={() => setSelectedId(el.id)}
                    className={`flex items-center gap-1.5 px-2 py-1.5 cursor-pointer text-[11px] hover:bg-gray-50 mx-1 rounded ${selectedId === el.id ? "bg-blue-50 shadow-sm font-medium text-blue-600 ring-1 ring-blue-200" : "text-gray-600"}`}>
            ...
            <TabsContent value="symbols" className="flex-1 overflow-y-auto m-0 p-2 space-y-2">
              <div className="flex items-center gap-1 flex-wrap">
                {(["all", "CE", "FDA", "NMPA"] as const).map(reg => (
                  <Button key={reg} variant={symbolFilter === reg ? "default" : "outline"} size="sm"
                    className={`h-5 text-[9px] px-2 ${symbolFilter === reg ? "bg-[#0F2B46]" : ""}`}
              ...
              {Object.entries(CATEGORY_LABELS).map(([cat, labels]) => {
                ...
                return (
                  <div key={cat}>
                    <p className="text-[9px] font-semibold text-gray-400 uppercase tracking-wide mb-1">{labels.zh}</p>
                    <div className="grid grid-cols-3 gap-1">
                      {syms.map(sym => (
                        <button key={sym.id} onClick={() => addElement("symbol", { symbolId: sym.id, width: 8, height: 8 })}
                          className="flex flex-col items-center gap-0.5 p-1.5 rounded border border-gray-200 bg-white hover:bg-blue-50 hover:border-blue-300 transition-colors">
                          {sym.render(18, "#333")}
                          <span className="text-[8px] text-gray-600 leading-tight text-center truncate w-full">{sym.nameZh}</span>
                        </button>
                      ))}
                    </div>
                  </div>
              ...
              {customSymbols.length > 0 && (
                <div>
                  <p className="text-[9px] font-semibold text-gray-400 uppercase tracking-wide mb-1">自定义符号</p>
                  <div className="grid grid-cols-3 gap-1">
                    {customSymbols.map(sym => (
                      <button key={sym.id} onClick={() => addElement("image", { imageSrc: sym.src, width: 8, height: 8 })}
                        className="flex flex-col items-center gap-0.5 p-1.5 rounded border border-gray-200 bg-white hover:bg-blue-50 hover:border-blue-300 transition-colors relative group">
                        <img src={sym.src} alt={sym.name} className="w-[18px] h-[18px] object-contain" />
                        <span className="text-[8px] text-gray-600 leading-tight text-center truncate w-full">{sym.name}</span>
        ...
        {/* 中间：画布 */}
        <div className="flex-1 overflow-auto flex items-start justify-center p-6 print:p-0 print:bg-white">
          <div className="flex flex-col items-center gap-1" style={{ transform: `scale(${canvasScale})`, transformOrigin: "top center" }}>
            ...
            <div className="flex">
              ...
              <div ref={canvasRef} onClick={() => !previewMode && setSelectedId(null)} className="label-print-canvas"
      ...
      {/* 底部状态栏 */}
      <div className="h-6 border-t bg-white flex items-center justify-between px-3 shrink-0 print:hidden">
        <span className="text-[10px] text-gray-400">元素: {template.elements.length} | 画布: {template.width}×{template.height}mm</span>
        <span className="text-[10px] text-gray-400">{selectedEl ? `选中: ${selectedEl.type} (${selectedEl.x}, ${selectedEl.y})` : "未选中元素"}</span>
      </div>
      ...
      <Dialog open={showPresetDialog} onOpenChange={setShowPresetDialog}>
        <DialogContent className="sm:max-w-[700px]">
          ...
          <div className="space-y-4 py-3">
            {[
              { reg: "MDR", title: "🇪🇺 欧盟 MDR 2017/745", desc: "需要CE标志、公告机构号、EC REP、Basic UDI-DI、SRN、ISO 15223符号", color: "border-blue-300 bg-blue-50/30" },
              { reg: "FDA", title: "🇺🇸 美国 FDA 21 CFR 801", desc: "需要Rx Only标识、FDA Listing号、GS1条码、制造商信息", color: "border-red-300 bg-red-50/30" },
              { reg: "NMPA", title: "🇨🇳 中国 NMPA", desc: "需要中文标注、注册证号、UDI-DI编码、生产企业信息", color: "border-green-300 bg-green-50/30" },
      ...
      <Dialog open={showNewSizeDialog} onOpenChange={setShowNewSizeDialog}>
        <DialogContent className="sm:max-w-[400px]">
          ...
          <DialogFooter>
            ...
            <Button size="sm" className="bg-[#0F2B46] hover:bg-[#0F2B46]/90" onClick={handleCreateNewSize}>创建模板</Button>
          </DialogFooter>
        </DialogContent>
      </Dialog>
